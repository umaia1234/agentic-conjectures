...
| [OEIS A067720 — phi(k^2+1)=k*phi(k+1) prime-power subfamily](problems/oeis-a067720/README.md) | 🟡 partial | lean | `GPT-5.6 Sol` | Lean-verifies that no solution has k+1=2^a with a>=2; an informal proof additionally shows that for odd prime p with V=v2(p^a-1)+v2(p-1)<=5 the only prime-power solution is (p,a,k)=(3,2,8). General composite k+1 and the V>=6 prime-power cases remain open. |
