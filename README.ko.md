...
| [OEIS A067720 — phi(k^2+1)=k*phi(k+1)의 소수 거듭제곱 부분족](problems/oeis-a067720/README.ko.md) | 🟡 부분 결과 | — | `GPT-5.6 Sol` | k+1=p^a이고 a>=2이면 p=2일 때 해가 없으며, V=v2(p^a-1)+v2(p-1)<=5인 홀수 p에 대해서는 (p,a,k)=(3,2,8)이 유일한 해임을 증명했다. 일반적인 합성수 k+1과 V>=6인 소수 거듭제곱 경우는 여전히 열려 있으므로 원래 문제는 미해결이다. |
