...
표시하고 있었다. 2026-08-13에는 같은 페이지의 상태가
**“Solution under review”**로 바뀌었다. 이는 제출물이 접수되었음을
뜻할 뿐 유효성을 확정하지 않으며, 페이지는 제출자를 밝히지 않는다.
고정된 정수 (1<n_1<n_2<\cdots<n_k)에 대해 다음 집합을 정의한다.
...
2026-08-13에는 공식 페이지의 세 상금 문제 모두가 “Solution under
review”로 바뀌었다. 제출물이나 제출자는 공개되지 않았으므로, 이 변경과
본 저장소 사이의 관계를 추정하지 않는다.
